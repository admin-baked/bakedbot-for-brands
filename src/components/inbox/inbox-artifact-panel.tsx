...
function CreativeDetail({ artifact }: { artifact: InboxArtifact }) {
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Media Preview */}
            <CreativeMediaPreview content={data} />

            <div className="grid grid-cols-2 gap-2 text-sm">
                {data.aspectRatio && (
                    <div className="p-3 rounded-lg bg-muted">
                        <div className="text-muted-foreground text-xs">Aspect Ratio</div>
                        <div className="font-medium">{data.aspectRatio}</div>
                    </div>
                )}
                {data.durationSeconds && (
                    <div className="p-3 rounded-lg bg-muted">
                        <div className="text-muted-foreground text-xs">Duration</div>
                        <div className="font-medium">{data.durationSeconds}s</div>
                    </div>
                )}
                <div className="p-3 rounded-lg bg-muted">
                    <div className="text-muted-foreground text-xs">Generated By</div>
                    <div className="font-medium capitalize">{data.generatedBy}</div>
                </div>
            </div>

            {/* Caption */}
            <div>
                <h4 className="font-medium text-sm mb-2">{data.caption ? 'Caption' : 'Prompt'}</h4>
                <p className="text-sm whitespace-pre-wrap">{data.caption || data.generationPrompt}</p>
            </div>
            ...
            {data.creativeDirection && (
                <div className="space-y-3">
                    <h4 className="font-medium text-sm">Creative Direction</h4>
                    {data.creativeDirection.title && (
                        <div className="p-3 rounded-lg bg-muted">
                            <div className="text-muted-foreground text-xs mb-1">Title</div>
                            <div className="text-sm">{data.creativeDirection.title}</div>
                        </div>
                    )}
                    {data.creativeDirection.hook && (
                        <div className="p-3 rounded-lg bg-muted">
                            <div className="text-muted-foreground text-xs mb-1">Hook</div>
                            <div className="text-sm">{data.creativeDirection.hook}</div>
                        </div>
                    )}
                    {data.creativeDirection.visuals && (
                        <div className="p-3 rounded-lg bg-muted">
                            <div className="text-muted-foreground text-xs mb-1">Visuals</div>
                            <div className="text-sm whitespace-pre-wrap">{data.creativeDirection.visuals}</div>
                        </div>
                    )}
                    {data.creativeDirection.audio && (
                        <div className="p-3 rounded-lg bg-muted">
                            <div className="text-muted-foreground text-xs mb-1">Audio</div>
                            <div className="text-sm whitespace-pre-wrap">{data.creativeDirection.audio}</div>
                        </div>
                    )}
                    {data.creativeDirection.script && (
                        <div className="p-3 rounded-lg bg-muted">
                            <div className="text-muted-foreground text-xs mb-1">Script</div>
                            <div className="text-sm whitespace-pre-wrap">{data.creativeDirection.script}</div>
                        </div>
                    )}
                </div>
            )}
