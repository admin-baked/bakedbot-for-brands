...
import type { PublicMenuSettings } from "@/types/public-menu-settings";
...
export function generateBrandLlmTxt(
  ...
  const lines: string[] = [];
  const brandType =
    brand.type === "dispensary" ? "Cannabis Dispensary" : "Cannabis Brand";
  ...
  if (products.length > 0) {
    ...
    if (products.length > MAX_PRODUCTS_IN_LLM_TXT) {
      lines.push(
        `> Showing ${MAX_PRODUCTS_IN_LLM_TXT} of ${products.length} products. See Agent API for full catalog: ${BASE_URL}/api/agent/${brandSlug}`,
      );
      lines.push("");
    }
  }
  ...
  if (bundles.length > 0) {
    ...
    for (const bundle of bundles) {
      const savings = bundle.savingsPercent
        ...
        : "";
      lines.push(
        `- ${bundle.name}: ${bundle.description || bundle.type}${savings}`,
      );
    }
    lines.push("");
  }
  ...
  lines.push(
    `- AI Budtender: ${BASE_URL}/${brandSlug} (chat available on menu page)`,
  );
  lines.push("");
...
export function generateGlobalLlmTxt(
  ...
  lines.push(
    "> The first cannabis commerce platform built for both the human web and the agent web.",
  );
  lines.push("");
  lines.push(
    "BakedBot AI powers dispensary and brand storefronts with AI agents,",
  );
  lines.push(
    "real-time POS integration, loyalty programs, and compliance automation.",
  );
  lines.push("");
  ...
  lines.push(
    "- Agent API: `GET /api/agent/{brandSlug}` — Returns JSON-LD (schema.org) structured product catalog",
  );
  lines.push(
    "- Per-brand LLM.txt: `GET /{brandSlug}/llm.txt` — Machine-readable brand/menu summary",
  );
  lines.push(
    "- Content-Type: `application/ld+json` (Agent API) | `text/plain` (llm.txt)",
  );
  lines.push("");
  ...
  lines.push(
    "- Real-time POS-synced product menus with terpene and cannabinoid profiles",
  );
  lines.push(
    "- AI budtender (Smokey) for personalized product recommendations",
  );
...
function buildLoyaltySection(settings: PublicMenuSettings): string[] {
  ...
  if (settings.tiers?.length) {
    ...
    for (const tier of settings.tiers) {
      const multiplier =
        tier.multiplier > 1 ? ` (${tier.multiplier}x points)` : "";
      lines.push(
        `  - ${tier.name}: ${tier.pointsRequired}+ points${multiplier}`,
      );
    }
  }
  ...
  if (settings.discountPrograms?.length) {
    const activePrograms = settings.discountPrograms.filter((p) => p.enabled);
