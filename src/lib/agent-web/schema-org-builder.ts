...
function mapProductToSchema(
  ...
  if (product.originalPrice != null && product.dynamicPricingApplied) {
    schema.offers.priceValidUntil = new Date(Date.now() + 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0]; // Valid for 24h
  }
  ...
  if (product.thcPercent != null) {
    props.push({
      "@type": "PropertyValue",
      name: "THC",
      value: product.thcPercent.toString(),
      unitCode: "P1",
    });
  }
  ...
  if (product.cbdPercent != null) {
    props.push({
      "@type": "PropertyValue",
      name: "CBD",
      value: product.cbdPercent.toString(),
      unitCode: "P1",
    });
  }
...
function buildLoyaltyPayload(
  ...
  if (settings.tiers?.length) {
    loyalty.tiers = settings.tiers.map((t) => ({
      name: t.name,
  ...
  if (settings.redemptionTiers?.length) {
    loyalty.redemptionOptions = settings.redemptionTiers.map((t) => ({
      pointsCost: t.points,
      dollarValue: t.value,
    }));
  }
  ...
  if (settings.discountPrograms?.length) {
    const active = settings.discountPrograms.filter((p) => p.enabled);
    if (active.length > 0) {
      loyalty.discountPrograms = active.map((p) => ({
        name: p.name,
...
function calculateDataCompleteness(products: Product[]): DataCompleteness {
  ...
  return {
    totalProducts: total,
    withTerpenes: products.filter((p) => p.terpenes && p.terpenes.length > 0)
      .length,
    withCannabinoids: products.filter(
      (p) => p.cannabinoids && p.cannabinoids.length > 0,
    ).length,
    withEffects: products.filter((p) => p.effects && p.effects.length > 0)
      .length,
    withImages: products.filter(
