...
// TODO: The 'generateReviewEmbeddings' tool was removed as it used outdated Genkit APIs.
// This script needs to be updated to call a new, valid Genkit flow for generating embeddings.
// For now, the main function is disabled to prevent build errors.

const main = async () => {
    console.log('🚀 Starting one-time initialization of review embeddings...');
    console.log('⚪️ SKIPPED: This script is currently disabled pending an update to the new Genkit embedding generation flow.');
    process.exit(0);
