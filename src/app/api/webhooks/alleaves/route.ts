...
export async function POST(request: NextRequest) {
    try {
        ...
        // Single Firestore client for the lifetime of this request
        ...
        switch (payload.event) {
            ...
            case 'order.created':
                handleOrderEvent(firestore, orgId, payload.data, 'order.created');
                // Dispatch to playbook system (fire-and-forget)
            ...
            case 'order.updated':
                handleOrderEvent(firestore, orgId, payload.data, 'order.updated');
                // Dispatch to playbook system (fire-and-forget)
...
async function handleOrderEvent(
    ...
    // Prefer locationId from the payload directly; skip location collection query
    const retailerId = orderData.locationId?.toString() || orgId;

    const rawDate = orderData.date_created || orderData.created_at || orderData.updated_at;
