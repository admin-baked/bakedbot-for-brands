/**
 * Enhanced Code Submission Form with Monaco Editor
 *
 * Provides VS Code-like editing experience for code challenges.
...
import { Loader2, Send, Code2, Sparkles } from 'lucide-react';
import { submitChallenge } from '@/server/actions/training';
import { useToast } from '@/hooks/use-toast';

// Lazy load Monaco Editor
const MonacoCodeEditor = lazy(() =>
    import('./monaco-code-editor').then((mod) => ({ default: mod.MonacoCodeEditor }))
);

interface CodeSubmissionFormMonacoProps {
    challengeId: string;
    cohortId: string;
    starterCode?: string;
...
export function CodeSubmissionFormMonaco({
    challengeId,
    cohortId,
    starterCode,
    attemptNumber,
}: CodeSubmissionFormMonacoProps) {
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                        ...
                        {useMonaco && <Sparkles className="h-4 w-4 text-primary" />}
                    </CardTitle>
                    <CardDescription>
                        {attemptNumber > 1 ? `Attempt #${attemptNumber}` : 'First attempt'} - Write your code below
                        {useMonaco && ' (Enhanced IDE)'}
                    </CardDescription>
                </CardHeader>

                <CardContent className="space-y-4">
                    {/* Code Input - Monaco or Textarea */}
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            ...
                            <Button
                                ...
                                className="text-xs"
                            >
                                {useMonaco ? 'Use Simple Editor' : 'Use Enhanced IDE'}
                            </Button>
                        </div>
                        ...
                        {useMonaco ? (
                            <Suspense
                                fallback={
                                    <Card className="flex items-center justify-center h-[500px] border-2 border-muted">
...
- Is this the right way to handle errors?"
                    ...
                    <Alert>
                        <AlertDescription className="text-sm">
                            ...
                            on code quality, TypeScript usage, and BakedBot standards.
                        </AlertDescription>
                    </Alert>
                </CardContent>
