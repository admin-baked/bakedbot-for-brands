...
export async function getPlatformAnalytics(): Promise<PlatformAnalyticsData> {
    try {
        ...
        const signupsToday = allUsers.filter((user) => user.signupAt >= todayStart).length;
        const signupsWeek = allUsers.filter((user) => user.signupAt >= sevenDaysAgo).length;
        const signupsMonth = allUsers.filter((user) => user.signupAt >= thirtyDaysAgo).length;

        const dailyActiveUsers = allUsers.filter((user) => user.lastLoginAt && user.lastLoginAt >= todayStart).length;
        const weeklyActiveUsers = allUsers.filter((user) => user.lastLoginAt && user.lastLoginAt >= sevenDaysAgo).length;
        const monthlyActiveUsers = allUsers.filter((user) => user.lastLoginAt && user.lastLoginAt >= thirtyDaysAgo).length;
