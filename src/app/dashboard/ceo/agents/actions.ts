...
const PLAYBOOK_REGISTRY: Record<string, () => Promise<PlaybookResult>> = {
    ...
    'competitor-takedown-daily': async () => {
        const logs: string[] = [];
        logs.push("Starting 'Competitor Takedown Strategy' Playbook...");
        
        // 1. Ezal - Discovery
        logs.push("Triggering Ezal for latest competitor intel...");
        const intelResult = await triggerAgentRun('ezal', "Find top 3 competitors in Chicago and their current deals.");
        logs.push(`Ezal Findings: ${intelResult.message}`);
        
        // 2. Big Worm - Analysis (Sidecar)
        logs.push("Handing off to Big Worm for Deep Strategy Analysis...");
        const strategyResult = await triggerAgentRun('bigworm', 
            `Analyze these competitors: ${intelResult.message}. Search for 'low cost guerrilla marketing tactics' to counter them. Run a 'competitor_analysis' projection.`);
        logs.push(`Big Worm Strategy: ${strategyResult.message}`);

        // 3. Report - Email Dispatch
        logs.push("Dispatching Strategy Report to CEO...");
        try {
            const { sendGenericEmail } = await import('@/lib/email/dispatcher');
            const emailBody = `
                <h1>Daily Competitor Takedown Strategy</h1>
                <p><strong>Date:</strong> ${new Date().toLocaleDateString()}</p>
                
                <h2>🕵️ Ezal's Intel</h2>
                <p>${intelResult.message}</p>
                
                <h2>🧠 Big Worm's Strategy</h2>
                <p>${strategyResult.message}</p>
                
                <hr/>
                <p><em>Generated by BakedBot AI</em></p>
            `;

            const emailResult = await sendGenericEmail({
                to: 'martez@bakedbot.ai',
                subject: '🎯 Daily Takedown Strategy Ready',
                htmlBody: emailBody,
                textBody: `Strategy: ${strategyResult.message}`
            });

            if (emailResult.success) {
                logs.push(`Email report sent successfully to martez@bakedbot.ai`);
            } else {
                logs.push(`Email dispatch failed: ${emailResult.error}`);
            }
        } catch (e: any) {
        ...
        return {
            success: true,
            message: "Daily Takedown Strategy Generated & Emailed.",
            logs
        };
    }
};
