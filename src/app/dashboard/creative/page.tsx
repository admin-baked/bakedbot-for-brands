...
export default function CreativeCommandCenter() {
  ...
  // Image mode: AI photo (FLUX.1) vs Branded (next/og text-on-color using brand guide colors)
  const [imageMode, setImageMode] = useState<'photo' | 'branded'>('photo');
                ...
                {activeLeftPanel === 'generate' && (
                  ...
                  <ScrollArea className="flex-1">
                    <div className="p-4 space-y-4">
                      ...
                      {/* Image mode toggle: AI Photo vs Branded */}
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Image Style</label>
                        <div className="flex rounded-lg border border-border bg-muted/40 p-0.5">
                          {(['photo', 'branded'] as const).map(m => (
                            <button
                              key={m}
