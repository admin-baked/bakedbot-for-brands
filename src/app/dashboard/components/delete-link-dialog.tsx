...
export default function DeleteLinkDialog({ isOpen, setIsOpen, link }: DeleteLinkDialogProps) {
  ...
  const handleDelete = () => {
    if (link) {
      ...
      toast({
        title: 'Link Removed',
        description: `The "${link.label}" link was successfully removed.`,
      });
      setIsOpen(false);
    }
  };
    ...
    <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This action cannot be undone. This will permanently delete the navigation link for <span className="font-bold">"{link?.label}"</span>.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
