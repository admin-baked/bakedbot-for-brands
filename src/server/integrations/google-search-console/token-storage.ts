...
const INTEGRATION_ID = 'google_search_console';

export async function saveGoogleSearchConsoleToken(userId: string, tokens: Credentials) {
...
export async function getGoogleSearchConsoleToken(userId: string): Promise<Credentials | null> {
  const { firestore } = await createServerClient();
  ...
  if (data.refreshTokenEncrypted) {
    try {
      ...
      console.error('Failed to decrypt Google Search Console refresh token', error);
      return null;
    }
  }
