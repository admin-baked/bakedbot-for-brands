...
export async function processSlackMessage(ctx: SlackMessageContext): Promise<void> {
    ...
    try {
        ...
        const AGENT_TIMEOUTS: Record<string, number> = {
            linus: 600_000,   // 10 min — Claude API + multi-tool (run_health_check, run_command)
            leo: 300_000,   // 5 min — COO operations may chain multiple tools
            jack: 300_000,   // 5 min — CRO revenue analysis
            glenda: 300_000,  // 5 min — CMO strategy
        };
        const agentTimeoutMs = AGENT_TIMEOUTS[personaId] ?? 90_000;
