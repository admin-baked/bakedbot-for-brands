...
// ============ Competitor Price Match Briefing Card ============

/**
 * A single product opportunity identified by Ezal for price matching.
 * Surfaces products where a competitor is priced ≤ ours on likely
 * foot-traffic drivers — and recommends matching or beating by $1.
 */
export interface PriceMatchOpportunity {
    productName: string;
    category: string;           // e.g., "Flower", "Vape", "Edible"
    ourPrice: number;
...
/**
 * Data payload for `competitor_price_match` artifacts.
 * Generated by PriceMatchInsightsGenerator (Ezal).
 */
export interface CompetitorPriceMatchData {
    date: string;               // YYYY-MM-DD
    opportunities: PriceMatchOpportunity[];
    totalSavingsGap: number;    // sum of (ourPrice - recommendedPrice) across all opps
    topCompetitor: string;      // competitor with most price advantages
    marketContext: string;      // e.g., "NJ Adult-Use | Competitive Market"
...
export function getArtifactTypesForThreadType(type: InboxThreadType): InboxArtifactType[] {
    const mapping: Record<InboxThreadType, InboxArtifactType[]> = {
        ...
        product_discovery: [],
        support: [],
        general: ['cohort_report', 'checkin_briefing'],
        // Super User: Growth Management
