 ...
 *
 * Run: npm run seed:compliance-kb
 */
...
import { writeComplianceFact } from '@/server/tools/hive-mind';
...
const facts: [string, string, string, string][] = [
    ...
    ['NY', 'packaging', 'NY packaging requirements: Child-resistant packaging required. No imagery appealing to minors. Must display: licensee name, license number, net weight, THC/CBD content, required warnings ("For Adults 21+", "Keep Away From Children"), batch number, and harvest/manufacture date.', SOURCE],
    ...
    ['CA', 'packaging', 'CA packaging: Child-resistant, re-sealable, opaque. Must include: CA universal cannabis symbol, DCC license number, "GOVERNMENT WARNING" statement, THC/CBD content, batch/lot number, weight, testing lab and date, ingredient list for edibles.', SOURCE],
    ['CA', 'delivery', 'CA cannabis delivery: Legal for licensed retailers (Type 9 delivery license). Retailer must have physical location in CA. Order must be received digitally. No cash on delivery (cashless ATM or card only).', SOURCE],
    ...
    ['CO', 'advertising', 'CO MED advertising: No mass market advertising where >30% audience is under 21. No billboards on interstate highways or within 1,000 feet of schools. All ads must include health warnings and license number. Radio/TV only on channels where 71.6%+ audience is 21+.', SOURCE],
    ['CO', 'packaging', 'CO packaging: Tamper-evident, child-resistant (CR certified). Must bear the CO universal symbol (exclamation mark in red diamond). Required labels: licensee name/number, net weight, THC/CBD %, "KEEP OUT OF REACH OF CHILDREN", batch number, test results.', SOURCE],
    ['CO', 'delivery', 'CO cannabis delivery: Legal with state-issued delivery endorsement. Requires GPS tracking, manifest system (METRC), and age verification at delivery. No delivery within 1,000 feet of a school.', SOURCE],
    ...
    ['IL', 'packaging', 'IL packaging: Opaque, child-resistant, tamper-evident. Must display: IDFPR license number, universal symbol, weight, THC/CBD content, "FOR USE BY ADULTS 21 AND OVER", batch number, manufacturer info, and health warnings.', SOURCE],
    ...
    ['MA', 'packaging', 'MA packaging: Child-resistant, tamper-evident, re-sealable. Opaque for single-use containers. Must include: licensee name/license number, CCC-required health warnings, net weight, THC/CBD %, batch ID, expiration/best-by date.', SOURCE],
    ...
    ['WA', 'advertising', 'WA LCB advertising rules: No advertising on channels or media primarily targeted to under-21 audience. No placement within 1,000 feet of schools, playgrounds, recreation centers, or public libraries. Must include LCB license number. No health claims. Social media requires age-gating.', SOURCE],
    ['WA', 'packaging', 'WA packaging: Exit packaging must be child-resistant. Must display: UBI number, pesticide disclosure, THC/CBD per serving and per package, number of servings, net weight, batch number, required warnings. Opaque exit bags required.', SOURCE],
    ['WA', 'delivery', 'WA cannabis delivery: Legal as of 2020 for licensed retailers. Requires LCB delivery endorsement. Must use LEAF Data Systems seed-to-sale. Age and ID verification at delivery. Only adult consumers may receive delivery — no delivery to businesses.', SOURCE],
    ...
    ['NV', 'advertising', 'NV CCB advertising rules: No advertising on media with >30% under-21 audience. No advertising within 1,000 feet of schools, playgrounds, or youth activity facilities. Must include "For Adults 21 and Over", "Keep Away From Children", and NV license number. No health claims.', SOURCE],
    ['NV', 'packaging', 'NV packaging: Child-resistant, tamper-evident, opaque. Required: CCB-issued license number, universal symbol, net weight, THC/CBD %, batch number, expiration date, "CONTAINS MARIJUANA — KEEP OUT OF REACH OF CHILDREN".', SOURCE],
    ['NV', 'delivery', 'NV cannabis delivery: Legal for licensed retailers with delivery endorsement from CCB. Must verify 21+ and valid ID at delivery. No delivery to vehicles or in public areas — must be a private residence. GPS tracking and electronic manifests required.', SOURCE],
    ...
    ['NJ', 'packaging', 'NJ packaging: Child-resistant, tamper-evident, opaque. Required labels: CRC license number, universal symbol, THC/CBD per serving and total, batch ID, expiration date, required health warnings.', SOURCE],
    ['NJ', 'delivery', 'NJ cannabis delivery: Legal with CRC delivery endorsement. Available to Class 5 (delivery) licensees and Class 1 retailers with endorsement. Age verification (21+) and government ID required at delivery. No delivery to public places. GPS tracking required.', SOURCE],
    ...
    ['MI', 'advertising', 'MI MRA advertising rules: No advertising on channels or platforms where 30%+ of audience is under 21. No outdoor advertising within 500 feet of schools, public playgrounds, or churches. Must include "For Adults 21+ Only", "Keep Out of Reach of Children", and MRA license number. No health claims.', SOURCE],
    ['MI', 'packaging', 'MI packaging: Child-resistant, tamper-evident, re-sealable. Opaque or not designed to appeal to minors. Must include: MRA licensee name/number, universal symbol, THC/CBD content, batch number, harvest/process date, weight, required health warnings.', SOURCE],
    ['MI', 'delivery', 'MI cannabis delivery: Legal for licensed retailers with state-issued delivery endorsement. Must use METRC for seed-to-sale tracking. Age and ID verification (21+) required at delivery. Delivery to residential addresses only.', SOURCE],
...
async function main(): Promise<void> {
    ...
    for (const [state, topic, rule, source] of facts) {
        try {
            ...
            process.stderr.write(`✗ ${state}/${topic}: ${msg}\n`);
        }
    ...
    if (errors > 0) {
        console.log('Re-run to retry failed facts (Letta deduplicates by content similarity).');
        process.exit(1);
    }
}
