...
- Fixed the Windows `safe-push` npm runner so the push gate can run `check:secrets` and `simplify:verify`.
...
- `cmd /c npm run simplify:record` recorded the outgoing code diff after both script fixes.
...
## Commit
- Code commits: `8040ec18e` (`fix: Harden Thrive CRM data and playbooks`), `b88970912` (`fix: Make safe push npm checks work on Windows`)
