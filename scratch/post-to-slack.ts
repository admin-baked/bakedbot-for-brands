import { elroySlackService } from './src/server/services/communications/slack';
...
`.trim();
  ...
  const result = await elroySlackService.postMessage(channel.id, 'Inventory Asset Audit Results', blocks);
  
  if (result.sent) {
    console.log(`✅ Message posted to #${channelName} (TS: ${result.ts})`);
  } else {
