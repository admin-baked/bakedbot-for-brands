...
class Awaiken_Elementor { 
	...
	public static function register_new_widgets( $widgets_manager ) {
		...
		require_once INCLUB_THEME_DIR . '/inc/compatibility/elementor/widgets/site-logo.php';
		require_once INCLUB_THEME_DIR . '/inc/compatibility/elementor/widgets/template.php';

		$widgets_manager->register( new \inclub_widget_site_Logo() );
		$widgets_manager->register( new \inclub_widget_template() );

	}
	
	public static function elementor_editor_styles() {
		wp_enqueue_style( 'inclub-elementor-editor', INCLUB_THEME_URL . '/assets/css/elementor-editor.css', array(), INCLUB_THEME_VERSION );
		...
		if ( !defined( 'ELEMENTOR_PRO_VERSION' ) ) {
			...
			wp_add_inline_style( 'inclub-elementor-editor', $custom_css );
		}
	}
	
	public static function elementor_preview_styles() {
		wp_enqueue_style( 'inclub-elementor-preview-style', INCLUB_THEME_URL . '/assets/css/elementor-preview.css', array(), INCLUB_THEME_VERSION );
	}
	
	public function update_default_elementor_kit() {

		add_option( 'default_inclub_kit', 0 );
		if ( get_option( 'default_inclub_kit' ) == 0 ) {
			...
			update_option( 'default_inclub_kit', 1 );
		}
	}
